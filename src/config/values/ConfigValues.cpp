...
#define MS makeConfigValue
    ...
    return std::vector<SP<IValue>>{
        ...
        MS<Int>("binds:movefocus_cycles_fullscreen", "If set to \"window\", it allows hl.dsp.focus({ direction }) to seek windows when focus is on a fullscreen window. If set to \"monitor\", it will seek just monitors instead.", 1, {.min = 0, .max = 1, .map = OptionMap{{"monitor", 0}, {"window", 1}}}),
